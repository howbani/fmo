...
using FMO.ExpermentsResults.Energy_consumptions;
using FMO.ExpermentsResults.Lifetime;
using FMO.ControlPlane.NOS.TC;
using FMO.ControlPlane.NOS.TC.subgrapgh;
using FMO.DataPlane.NeighborsDiscovery;
using FMO.ControlPlane.NOS.FlowEngin;
using FMO.ui.conts;
using FMO.Charts.Intilization;
using FMO.ControlPlane.NOS.Visualizating;
    ...
    public partial class MainWindow : Window
    {
        ...
        private void TimerCounter_Tick(object sender, EventArgs e)
        {
            if (PublicParamerters.SimulationTime <= stopSimlationWhen + PublicParamerters.MacStartUp)
            {
                ...
                Dispatcher.Invoke(() => Title = "FMO:" + PublicParamerters.SimulationTime.ToString(), DispatcherPriority.Send);
                //the forwarderPath of nodes are updated periodiaclly.
                if (PublicParamerters.SimulationTime % PublicParamerters.UpdateFMOeveryXseconds == 0)
                {
                    TimerCounter.Stop();
                    RandomSelectSourceNodesTimer.Stop();
                    FmoNetwork fmo = new FmoNetwork(myNetWork);
                    fmo.UpdateForwardPath();
                    TimerCounter.Start();
        ...
        public void HideSimulationParameters()
        {
            ...
            lbl_Overhead_packets.Content = "0";
        ...
        public void RandomDeplayment(int sinkIndex)
        {
            ...
            PublicParamerters.PowersString = PublicParamerters.NetworkName;
            ...
            PublicParamerters.myNetWork = new List<Sensor>();
            PublicParamerters.myNetWork = myNetWork;
            PublicParamerters.myNetWorkSize = myNetWork.Count;
            FmoNetwork initFmo = new FmoNetwork(myNetWork);
            initFmo.compute();
            //PrintInfos.PrintForwardPathInfo(myNetWork);
            //PrintInfos.PrintWeightOfEachEdgesInfo(myNetWork);
            //PrintInfos.PrintForwardersInfo(myNetWork);
        ...
        public void ClearExperment()
        {
            try
            {
                ...
                PublicParamerters.NumberofDeliveredDataPacket = 0;
                ...
                PublicParamerters.NumberofUpdatePackets = 0;
