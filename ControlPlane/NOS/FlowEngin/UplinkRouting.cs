...
namespace FMO.ControlPlane.NOS.FlowEngin
{
    ...
    public class UplinkRouting
    {
        ...
        public static void UpdateUplinkFlowEnery(Sensor sender,int nextNodeID)
        {
            sender.MiniFlowTable.Clear();
            ComputeUplinkFlowEnery(sender,nextNodeID);    
        }
        public static void ComputeUplinkFlowEnery(Sensor sender,int nextNodeID) 
        {
            foreach (NeighborsTableEntry neiEntry in sender.NeighborsTable) 
            {
                if (neiEntry.NeiNode.ResidualEnergyPercentage >= 0 && neiEntry.NeiNode.ID==nextNodeID) // the node is a live.
                {
        ...
        public static void ComputeUplinkFlowEneryInit(Sensor sender)
        {
